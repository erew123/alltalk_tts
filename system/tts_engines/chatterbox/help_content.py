...
class AllTalkHelpContent:
    ...
    HELP_PAGE = """
    ## 🎯 Chatterbox TTS Engine Help

    Chatterbox TTS is a neural text-to-speech system that supports voice cloning via audio prompts. It automatically downloads models and provides high-quality speech generation with cross-platform support.
    """
    ...
    - Always uses the latest compatible models

    ### Voice Cloning System
    - Zero-shot voice cloning via audio prompts
    - Support for custom voice samples
    ...
    - **CPU**: Universal CPU fallback
    - Automatic optimal device detection

    ## 🎵 Voice Configuration

    ### Using Audio Prompts
    - Place voice samples in `/alltalk_tts/voices/` directory
    ...
    1. **Default Generation**: No voice specified uses default model voice
    2. **Audio Prompt Cloning**: Specify WAV file path as voice parameter
    3. **API Integration**: Pass voice file paths through API calls
    ...
    ### Low VRAM Mode
    - Automatically moves model between CPU and GPU memory
    - Ideal for systems with limited VRAM
    - Useful when running alongside LLMs or other GPU applications
    - Enable in Default Settings tab

    ### Device Selection Priority
    1. **CUDA** (if available): Best performance
    2. **MPS** (Apple Silicon): Optimized for Mac
    ...
    1. **Voice Samples**
        ...
        - Keep samples between 3-15 seconds
        - Use consistent audio levels

    2. **Performance**
        - Enable Low VRAM for memory-constrained systems
        - Monitor GPU temperature during long generations
        - Clear GPU cache if experiencing memory issues

    3. **Quality**
        - Test different voice samples for best results
        - Use clear, well-articulated speech
        - Consider emotional tone of reference audio

    ## 📁 File Management

    - **Voice Samples**: `/alltalk_tts/voices/`
    - **Generated Audio**: `/alltalk_tts/outputs/`
    - **Models**: Automatically managed by Chatterbox
    - **Cache**: Handled transparently

    ## 🔧 System Integration

    ### API Compatibility
    - Full AllTalk API support
    - OpenAI-compatible endpoints
    - Voice parameter accepts file paths
    - Streaming not currently supported

    ### Limitations
    - No streaming capability
    - No temperature/repetition penalty controls
    - No pitch adjustment
    ...
    - Voice cloning requires WAV format audio prompts
