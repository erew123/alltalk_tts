...
warnings.filterwarnings(
    "ignore", message="Failed to launch Triton kernels, likely due to missing CUDA toolkit")
# Try to import faster-whisper, install if not found
try:
    from faster_whisper import WhisperModel
except ImportError:
    print("[AllTalk TTSDiff] Faster-Whisper not found. Attempting to install...")
    try:
        subprocess.check_call(
            [sys.executable, "-m", "pip", "install", "faster-whisper"])
        from faster_whisper import WhisperModel
        print("[AllTalk TTSDiff] Successfully installed Faster-Whisper!")
    except Exception as e:
        print("[AllTalk TTSDiff] Failed to install Faster-Whisper:")
        print(f"[AllTalk TTSDiff] Error: {str(e)}")
        print(
            "[AllTalk TTSDiff] Please try manually installing with: pip install faster-whisper")
        sys.exit(1)

parser = argparse.ArgumentParser(
    description="Compare TTS output with the original text using detailed comparison.")
parser.add_argument("--threshold", type=int, default=98,
                    help="Similarity threshold for considering a match (default: 98)")
...
if not args.ttslistpath:
    parser.error(
        "[AllTalk TTSDiff] Please specify the path to the ttslist.json file using the --ttslistpath argument.")
...
# Capture the model name from the argument
...
try:
    ...
    try:
        ...
        print(
            "[AllTalk TTSDiff] Attempting to automatically download the spaCy language model.")
        subprocess.run([sys.executable, "-m", "spacy",
                       "download", "en_core_web_md"], check=True)
        ...
        print(
            "[AllTalk TTSDiff] Please manually install the spaCy language model by running:")
...
def texts_are_similar(text1, text2, threshold=0.8):
    ...
    if len(text1) < 10 or len(text2) < 10:
        # Use FuzzyWuzzy for short texts
...
def transcribe_and_compare(file_url, original_text, model, item_id, flagged_ids):
    ...
    if audio_file_path is None:
        print(
            f"[AllTalk TTSDiff] Could not access local file for URL: {file_url}")
    ...
    try:
        ...
        print(
            f"[AllTalk TTSDiff] Error transcribing file {audio_file_path}: {e}")
    ...
    is_detailed_match = detailed_comparison(
        original_text_normalized, transcribed_text_normalized, args.threshold)
    has_spoken_punctuation = contains_spoken_punctuation(
        transcribed_text_normalized) and not contains_spoken_punctuation(original_text_normalized)
    ...
    if not is_match:
        ...
        if has_spoken_punctuation:
            print(
                f"[AllTalk TTSDiff] Note: Potential incorrect spoken punctuation detected.")
...
def main():
    ...
    print("[AllTalk TTSDiff] \033[94mJSON file  :\033[92m",
          json_file_path, "\033[0m")
    print("[AllTalk TTSDiff] \033[94mWAV files  :\033[92m",
          wav_file_path, "\033[0m")
    print("[AllTalk TTSDiff] \033[94mAccuracy   :\033[92m",
          accuracy_threshold, "%", "\033[0m")
    print("[AllTalk TTSDiff] \033[94mWhisper    :\033[92m",
          whisper_model_name, "\033[0m")

    device = "cuda" if torch.cuda.is_available() else "cpu"
    print("[AllTalk TTSDiff] \033[94mDevice     :\033[92m", device, "\033[0m")
    print("[AllTalk TTSDiff] Loading Faster-Whisper model...")
    model = WhisperModel(whisper_model_name, device=device,
    ...
    try:
        ...
        print(
            "[AllTalk TTSDiff] The JSON file must be named \033[93mttsList.json\033[0m")
        exit(1)
    ...
    for item in tts_list:
        ...
        transcribe_and_compare(
            item['fileUrl'], item['text'], model, item['id'], flagged_ids)

    print(f"[AllTalk TTSDiff]")
    # Print summary information at the end
    if flagged_ids:
        print(f"[AllTalk TTSDiff] \033[94mIDs needing review:\033[0m",
              ', '.join(map(str, flagged_ids)))
        print(f"[AllTalk TTSDiff]")
