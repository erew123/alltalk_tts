...
3) In your `Anaconda Prompt` command prompt/terminal, move into the folder where you want to download AllTalk to and then git clone this repository. For simplicity I am going to assume that location is `c:\myfiles\`<br><br>`cd myfiles`<br><br>`git clone https://github.com/erew123/alltalk_tts`
...
5) **If you are NOT using an Nvidia card and Cuda, skip to step 6** We will need to force PyTorch to install with CUDA.<br><br>`pip cache purge` (Will clear out all current copies of downloads that pip has)<br>`pip uninstall torch torchaudio` (Will uninstall the the current version of Torch and Torchaudio from the (alltalkenv) environment)<br><br>
